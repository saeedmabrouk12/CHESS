...
#include<conio.h>
#define length 8
char chessboard[length][length],killed[50];
char *numofmoves=chessboard,*numofkilled=killed;
int moves[250][4];
int* urmoves=moves;
void fullchess();
    ...
    if (whoplay%2==0&&chessboard[a][b]>65&&chessboard[a][b]<83){
            printf("NOT Valid. White Should Play");
                ...
                printchessboard();
                    ...
                    printf("NOT Valid. Plack Should Play");
                    while (getchar()!='\n');
                    while (getchar()!='\n');
                    printchessboard();
                }
...
bool isempty (int a, int b){
    if (chessboard[a][b]=='-'||chessboard[a][b]=='.') return true;
    return false;
}
bool checkwhitebowns (int a,int b,int c,int d){
    if (c-a==2){
...
bool checkking (int a,int b,int c,int d){
    if (abs(a-c)==abs(d-b)==1)  return true;
    if (a-c==0&&abs(d-b)==1) return true;
    if (abs(a-c)==1&&d-b==0)  return true;
    else return false;
